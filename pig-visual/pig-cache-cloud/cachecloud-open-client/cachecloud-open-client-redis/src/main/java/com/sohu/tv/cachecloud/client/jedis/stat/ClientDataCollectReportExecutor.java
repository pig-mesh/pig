...
public class ClientDataCollectReportExecutor {
    ...
    private ClientDataCollectReportExecutor(String url) {
        init(url);
    }

    public static ClientDataCollectReportExecutor getInstance(String url) {
        if (jedisDataCollectAndReportExecutor == null) {
            synchronized (ClientDataCollectReportExecutor.class) {
                if (jedisDataCollectAndReportExecutor == null) {
                    jedisDataCollectAndReportExecutor = new ClientDataCollectReportExecutor(url);
                }
            }
        }
        ...
        Thread clientDataCollectReportThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    collectReportAllData(url,currentMinuteStamp);
                } catch (Exception e) {
     ...
     */
    private void collectReportAllData(String url,String currentMinuteStamp) {
    	//1. 获取上一分钟的所有数据
		...
		if (!list.isEmpty()) {
	        ...
	        ClientReportDataCenter.reportData(url,ccReportBean);
		}
	}
