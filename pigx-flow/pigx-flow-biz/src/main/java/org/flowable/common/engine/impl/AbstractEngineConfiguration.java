...
public abstract class AbstractEngineConfiguration {
	...
	/**
	 * Checks the version of the DB schema against the library when the form engine is
	...
	/**
	 * Creates the schema when the form engine is being created and drops the schema when
	 * the form engine is being closed.
	 */
	public static final String DB_SCHEMA_UPDATE_DROP_CREATE = "drop-create";

	/**
	 * Upon building of the process engine, a check is performed and an update of the
	 * schema is performed if it is necessary.
	 */
	...
	/**
	 * Whether to use a lock when performing the database schema create or update
	 * operations.
	 */
	...
	/**
	 * Some databases have a limit of how many parameters one sql insert can have (eg SQL
	 * Server, 2000 params (!= insert statements) ). Tweak this parameter in case of
	 * exceptions indicating too much is being put into one bulk insert, or make it higher
	 * if your database can cope with it and there are inserts with a huge amount of data.
	 * <p>
	 * By default: 100 (55 for mssql server as it has a hard limit of 2000 parameters in a
	 * statement)
	 */
	protected int maxNrOfStatementsInBulkInsert = 100;

	public int DEFAULT_MAX_NR_OF_STATEMENTS_BULK_INSERT_SQL_SERVER = 55; // currently
																			// Execution
																			// has most
																			// params
																			// (35). 2000
	...
	/**
	 * Flag that can be set to configure or not a relational database is used. This is
	 * useful for custom implementations that do not use relational databases at all.
	 *
	 * If true (default), the
	 * {@link AbstractEngineConfiguration#getDatabaseSchemaUpdate()} value will be used to
	 * determine what needs to happen wrt the database schema.
	 *
	 * If false, no validation or schema creation will be done. That means that the
	 * database schema must have been created 'manually' before but the engine does not
	 * validate whether the schema is correct. The
	 * {@link AbstractEngineConfiguration#getDatabaseSchemaUpdate()} value will not be
	 * used.
	 */
	protected boolean usingRelationalDatabase = true;

	/**
	 * Flag that can be set to configure whether or not a schema is used. This is useful
	 * for custom implementations that do not use relational databases at all. Setting
	 * {@link #usingRelationalDatabase} to true will automatically imply using a schema.
	 */
	 ...
	 * of the process engine. For example, if you specify a prefix named 'PRE1.', Flowable
	 * will query for executions in a table named 'PRE1.ACT_RU_EXECUTION_'.
	 *
	 * <p>
	 * <strong>NOTE: the prefix is not respected by automatic database schema management.
	 * If you use {@link AbstractEngineConfiguration#DB_SCHEMA_UPDATE_CREATE_DROP} or
	 * {@link AbstractEngineConfiguration#DB_SCHEMA_UPDATE_TRUE}, Flowable will create the
	...
	/**
	 * In some situations you want to set the schema to use for table checks / generation
	 * if the database metadata doesn't return that correctly, see
	...
	/**
	 * Set to true in case the defined databaseTablePrefix is a schema-name, instead of an
	 * actual table name prefix. This is relevant for checking if Flowable-tables exist,
	 * the databaseTablePrefix will not be used here - since the schema is taken into
	 * account already, adding a prefix for the table-check will result in wrong
	 * table-names.
	 */
	protected boolean tablePrefixIsSchema;

	/**
	 * Set to true if the latest version of a definition should be retrieved, ignoring a
	 * possible parent deployment id value
	 */
	protected boolean alwaysLookupLatestDefinitionVersion;

	/**
	 * Set to true if by default lookups should fallback to the default tenant (an empty
	 * string by default or a defined tenant value)
	 */
	protected boolean fallbackToDefaultTenant;

	/**
	 * Default tenant provider that is executed when looking up definitions, in case the
	 * global or local fallback to default tenant value is true
	 */
	...
	protected boolean enableConfiguratorServiceLoader = true; // Enabled by default. In
	...
	protected List<EngineConfigurator> allConfigurators; // Including auto-discovered
															// configurators
	...
	public static Properties getDefaultDatabaseTypeMappings() {
		...
		databaseTypeMappings.setProperty("KingbaseES", DATABASE_TYPE_POSTGRES);// 扩展支持人大金仓
	...
	/**
	 * Define a max length for storing String variable types in the database. Mainly used
	 * for the Oracle NVARCHAR2 limit of 2000 characters
	 */
	...
	public void initDatabaseType() {
		...
		try {
			...
			// CRDB does not expose the version through the jdbc driver, so we need to
			// fetch it through version().
	...
	public void initEntityManagers() {
		...
		if (byteArrayEntityManager == null) {
			byteArrayEntityManager = new ByteArrayEntityManagerImpl(byteArrayDataManager, getEngineCfgKey(),
	...
	public void initSqlSessionFactory() {
		if (sqlSessionFactory == null) {
			...
			// When custom XML Mappers are registered with this engine they need to be
			// loaded in the configuration as well
	...
	public void initMybatisTypeHandlers(Configuration configuration) {
		...
		// Therefore we are registering the mappings between Object.class and the specific
		// jdbc type here.
	...
	public void initConfigurators() {
		...
		if (enableConfiguratorServiceLoader) {
			...
			ServiceLoader<EngineConfigurator> configuratorServiceLoader = ServiceLoader.load(EngineConfigurator.class,
					classLoader);
	...
	public void close() {
		if (forceCloseMybatisConnectionPool && dataSource instanceof PooledDataSource) {
			/*
			 * When the datasource is created by a Flowable engine (i.e. it's an instance
			 * of PooledDataSource), the connection pool needs to be closed when closing
			 * the engine. Note that calling forceCloseAll() multiple times (as is the
			 * case when running with multiple engine) is ok.
			 */
	...
	public AbstractEngineConfiguration setEngineLifecycleListeners(
			List<EngineLifecycleListener> engineLifecycleListeners) {
	...
	public AbstractEngineConfiguration setJdbcDefaultTransactionIsolationLevel(
	...
	public AbstractEngineConfiguration setCustomPreCommandInterceptors(
	...
	public AbstractEngineConfiguration setCustomPostCommandInterceptors(
	...
	public AbstractEngineConfiguration setEngineConfigurations(
	...
	public AbstractEngineConfiguration setServiceConfigurations(
	...
	public AbstractEngineConfiguration setEventRegistryEventConsumers(
	...
	public AbstractEngineConfiguration setDefaultCommandInterceptors(
	...
	public AbstractEngineConfiguration setTransactionContextFactory(
	...
	public AbstractEngineConfiguration setDependentEngineMyBatisXmlMappers(
			Set<String> dependentEngineMyBatisXmlMappers) {
	...
	public AbstractEngineConfiguration setAlwaysLookupLatestDefinitionVersion(
			boolean alwaysLookupLatestDefinitionVersion) {
	 ...
	 * @deprecated use {@link AbstractEngineConfiguration#getDefaultTenantProvider()}
	 * instead
	 */
	@Deprecated
	...
	public AbstractEngineConfiguration setTypedEventListeners(
	...
	public AbstractEngineConfiguration setAdditionalEventDispatchActions(
	...
	/**
	 * @return All {@link EngineConfigurator} instances. Will only contain values after
	 * init of the engine. Use the {@link #getConfigurators()} or
	 * {@link #addConfigurator(EngineConfigurator)} methods otherwise.
	 */
