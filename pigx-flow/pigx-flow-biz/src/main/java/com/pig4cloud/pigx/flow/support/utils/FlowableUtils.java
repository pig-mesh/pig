...
import com.pig4cloud.pigx.flow.dto.BizFlowBaseDto;
 ...
 * Flowable流程引擎工具类
 * <p>
 * 提供流程模型遍历、节点查找、路径分析等功能的工具类。
 * 该类是流程引擎操作的核心工具，支持复杂的流程拓扑结构分析。
 * <p>
 * 主要功能：
 * 1. 流程元素的入口/出口连线获取
 * 2. 递归遍历流程定义获取所有元素
...
public class FlowableUtils {
     ...
     * 根据节点获取入口连线集合
     * <p>
     * 支持多种流程元素类型，包括任务、网关、子流程、开始/结束事件等。
     * 入口连线是指向该节点的所有连线。
     *
     * @param source 流程元素节点
     * @return 入口连线集合，如果节点类型不支持则返回null
     */
     ...
     *
     * @param source 流程元素节点
     * @return 出口连线集合，如果节点类型不支持则返回null
     */
     ...
     * 递归获取流程定义所有的流程元素
     * <p>
     * 深度优先遍历流程定义，包括子流程中的所有元素。
     * 该方法常用于分析整个流程结构或查找特定元素。
     *
     * @param flowElements 当前级别流程元素集合
     * @param allElements  用于存储的全部流程元素集合，可为null
     * @return 全部流程元素集合，包含所有层级的元素
     */
     ...
     * 从后向前递归获取父级用户任务节点
     * <p>
     * 从当前节点开始，沿着入口连线向前查找所有的用户任务节点。
     * 主要用于流程回退、驳回等场景，找到可以回退的目标节点。
     * 支持子流程的遍历。
     *
     * @param source          当前节点
     * @param hasSequenceFlow 已处理过的顺序流集合，避免循环，可为null
     * @param userTaskList    递归获取的用户任务节点集合，可为null
     * @return 用户任务节点集合
     */
    ...
    /**
     * 根据正在运行的任务节点，迭代获取子级任务节点列表
     * <p>
     * 从当前节点开始，沿着出口连线向后查找正在运行的任务节点。
     * 主要用于找到需要撤回或影响的下游节点。
     *
     * @param source          起始节点
     * @param runActiveIdList 正在运行的任务Key，用于校验任务节点是否是正在运行的节点
     ...
     * 迭代获取子流程用户任务节点
     * <p>
     * 专门用于处理子流程内部的用户任务节点查找。
     * 从子流程的起始节点开始，找到所有的用户任务节点。
     *
     * @param source          起始节点
     * @param hasSequenceFlow 已经经过的连线的ID，用于判断线路是否重复
     ...
     * 从后向前寻路，获取所有脏线路上的点
     * <p>
     * 脏线路是指由于流程回退、驳回等操作产生的废弃路径。
     * 该方法用于识别这些废弃路径上的所有节点，以便进行清理或过滤。
     *
     * @param source          起始节点
     * @param passRoads       已经经过的点集合
     * @param hasSequenceFlow 已经经过的连线的ID，用于判断线路是否重复
     * @param targets         目标脏线路终点
     * @param dirtyRoads      确定为脏数据的点，因为不需要重复，因此使用set存储
     * @return 脏线路上的所有节点ID集合
     */
     ...
     * 迭代获取子流程脏路线
     * <p>
     * 当回退点是子流程时，子流程中的所有节点都被认为是脏路线。
     * 该方法递归遍历子流程，获取其中的所有节点。
     *
     * @param source          起始节点
     * @param hasSequenceFlow 已经经过的连线的ID，用于判断线路是否重复
     * @param dirtyRoads      确定为脏数据的点，因为不需要重复，因此使用set存储
     * @return 子流程中所有节点的ID集合
     */
     ...
     * 判断脏路线结束节点是否在子流程上
     * <p>
     * 用于判断脏路线的范围，如果脏路线终点在子流程中，
     * 说明脏路线只到子流程为止，不会影响父流程。
     *
     * @param source          起始节点
     * @param hasSequenceFlow 已经经过的连线的ID，用于判断线路是否重复
     * @param targets         判断脏路线节点是否存在子流程上，只要存在一个，说明脏路线只到子流程为止
     * @param inChildProcess  是否存在子流程上，true是，false否
     * @return 是否在子流程中
     */
     ...
     * <p>
     * 用于判断两个节点之间是否存在并行关系。
     * 如果两个节点之间只有唯一路径，则为串行；如果存在多条路径，则为并行。
     * 不存在直接回退到子流程中的情况，但存在从子流程出去到父流程情况。
     *
     * @param source          起始节点
     * @param targetKsy       目标节点ID
     * @param hasSequenceFlow 已经经过的连线的ID，用于判断线路是否重复
     * @param isSequential    是否串行，默认为true
     * @return true表示串行，false表示并行
     */
    ...
    /**
     * 历史节点数据清洗，清洗掉由于回滚导致的脏数据
     * <p>
     * 在流程回退、驳回等操作后，历史数据中会存在废弃的节点记录。
     * 该方法通过分析删除原因和节点关系，识别并过滤掉这些脏数据，
     * 返回真实有效的历史节点记录。
     * <p>
     * 特别处理：
     * - 会签节点的多实例情况
     ...
     * 获取流程参数Map，并设置发起人信息
     *
     * @param flowBaseDto 流程基础数据传输对象
     * @return 包含发起人信息的流程参数Map
     */
    public static Map<String, Object> getFlowParamMap(BizFlowBaseDto flowBaseDto) {
        Map<String, Object> paramMap = flowBaseDto.getFlowParamMap();
        // 设置发起人 (当前登录用户)
