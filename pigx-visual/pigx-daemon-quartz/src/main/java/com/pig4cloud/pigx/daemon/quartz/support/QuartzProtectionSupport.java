package com.pig4cloud.pigx.daemon.quartz.support;

import cn.hutool.core.util.IdUtil;
import com.pig4cloud.pigx.common.data.cache.RedisUtils;
import com.pig4cloud.pigx.common.data.tenant.TenantContextHolder;
import com.pig4cloud.pigx.daemon.quartz.config.QuartzProtectionProperties;
 ...
 * Quartz 重复触发保护支撑组件。
 * <p>
 * 该组件负责封装 Quartz 防重逻辑所需的 Redis 锁、触发元数据生成和锁释放流程，
 * 让调度入口与任务执行入口都能共享同一套保护规则。
 * </p>
 */
@Component
...
public class QuartzProtectionSupport {
    ...
    public QuartzExecutionMetadata buildExecutionMetadata(JobExecutionContext jobExecutionContext, Trigger trigger) {
        ...
        return QuartzExecutionMetadata.of(scheduledFireTime, jobExecutionContext.getFireInstanceId(), trigger, "QUARTZ");
    }
    ...
    public ProtectionLock tryAcquireFireDedupLock(SysJob sysJob, QuartzExecutionMetadata metadata) {
        ...
        String lockKey = FIRE_DEDUP_KEY_PREFIX + resolveTenantId(sysJob) + ":" + sysJob.getJobId() + ":"
                + metadata.scheduledFireTime().getTime();
    ...
    public ProtectionLock tryAcquireRunningLock(SysJob sysJob) {
        ...
        String token = IdUtil.fastSimpleUUID();
        String lockKey = RUNNING_LOCK_KEY_PREFIX + resolveTenantId(sysJob) + ":" + sysJob.getJobId();
     ...
     *
     * @param sysJob 当前任务配置，不允许为空
     * @return 任务显式租户ID，若未配置则回退到当前线程租户上下文
     */
    private Long resolveTenantId(SysJob sysJob) {
        return Objects.nonNull(sysJob.getTenantId()) ? sysJob.getTenantId() : TenantContextHolder.getTenantId();
    }
