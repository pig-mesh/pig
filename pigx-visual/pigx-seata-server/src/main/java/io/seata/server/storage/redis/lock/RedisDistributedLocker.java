...
public class RedisDistributedLocker implements DistributedLocker {
	...
	public boolean acquireLock(DistributedLockDO distributedLockDO) {
		try (Jedis jedis = JedisPooledFactory.getJedisInstance()) {
			...
			String result = jedis.set(distributedLockDO.getLockKey(), distributedLockDO.getLockValue(),
					SetParams.setParams().nx().px(distributedLockDO.getExpireTime()));
			return SUCCESS.equalsIgnoreCase(result);
		}
		catch (Exception ex) {
			LOGGER.error("The {} acquired the {} distributed lock failed.", distributedLockDO.getLockValue(),
					distributedLockDO.getLockKey(), ex);
			return false;
		}
	}
	...
	public boolean releaseLock(DistributedLockDO distributedLockDO) {
		...
		try (Jedis jedis = JedisPooledFactory.getJedisInstance()) {
			...
			// The lock hold by others,If other one get the lock,we release lock success
			// too as for current lockKey
			jedis.unwatch();
			return true;
		}
		catch (Exception ex) {
