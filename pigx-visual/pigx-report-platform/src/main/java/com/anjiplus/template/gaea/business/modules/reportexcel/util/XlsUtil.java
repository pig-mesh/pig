...
/**
 * 来自：https://github.com/mengshukeji/LuckysheetServer 使用poi导出xls
 *
...
public class XlsUtil {
	 ...
	 */
	public static void exportXlsFile(OutputStream outputStream, Boolean isXlsx, List<JSONObject> dbObjectList)
			throws IOException {
		Workbook wb = null;
		if (isXlsx) {
			wb = new XSSFWorkbook();
	...
	public static GridRecordDataModel strToModel(String list_id, String index, int status, int order) {
		...
		strSheet = strSheet.replace("reSheetName", "Sheet" + index).replace("reIndex", index)
				.replace("reOrder", order + "").replace("reStatus", status + "");
				...
				for (int j = cellAddresses.getFirstColumn(); j <= cellAddresses.getLastColumn(); j++) {
					if (i == cellAddresses.getFirstRow() && j == cellAddresses.getFirstColumn()) {
						...
						rangeMap.put(i + "_" + j, cellAddresses.getFirstRow() + "_" + cellAddresses.getFirstColumn()
								+ "_" + cellAddresses.getLastRow() + "_" + cellAddresses.getLastColumn());
					}
					else {
						rangeMap.put(i + "_" + j, cellAddresses.getFirstRow() + "_" + cellAddresses.getFirstColumn());
					}
	 ...
	 * @date 2021/4/21
	 */
	private static void dealWithExcelStyle(GridRecordDataModel model, JSONObject dataModel, Cell cell, Sheet sheet,
			Workbook workbook) {
		// 设置单元格合并信息
		...
		if (cellStyle.getBorderTop().getCode() > 0 || cellStyle.getBorderBottom().getCode() > 0
			...
			if (cellStyle instanceof XSSFCellStyle) {
				...
				if (Objects.equals((short) 0, cellStyle.getBorderTop().getCode())) {
					...
					border.getJSONObject("value").getJSONObject("t").put("color",
							dealWithRbg(xssfCellStyle.getTopBorderXSSFColor().getRGB()));
				}
				if (Objects.equals((short) 0, cellStyle.getBorderRight().getCode())) {
					border.getJSONObject("value").remove("r");
				}
				else {
					border.getJSONObject("value").getJSONObject("r").put("color",
							dealWithRbg(xssfCellStyle.getRightBorderXSSFColor().getRGB()));
				}
				if (Objects.equals((short) 0, cellStyle.getBorderLeft().getCode())) {
					border.getJSONObject("value").remove("l");
				}
				else {
					border.getJSONObject("value").getJSONObject("l").put("color",
							dealWithRbg(xssfCellStyle.getLeftBorderXSSFColor().getRGB()));
				}
				if (Objects.equals((short) 0, cellStyle.getBorderBottom().getCode())) {
					border.getJSONObject("value").remove("b");
				}
				else {
					border.getJSONObject("value").getJSONObject("b").put("color",
							dealWithRbg(xssfCellStyle.getBottomBorderXSSFColor().getRGB()));
				}
			}
	...
	private static void setFontStyle(JSONObject dataModel, Workbook workbook, Cell cell) {
		...
		v.put("ht", ExcelCenterStyleEnum.getExcelCenterStyleByExcelCenterCode(cellStyle.getAlignment().getCode())
				.getOnlineExcelCode());
		// bl 字体加粗设置
		...
		if (font instanceof HSSFFont) {
			...
			if (Objects.nonNull(hssfColor)) {
				v.put("fc", ColorUtil.convertRGBToHex(hssfColor.getTriplet()[0], hssfColor.getTriplet()[1],
