...
public class ReportExcelServiceImpl implements ReportExcelService {
	 ...
	 * @param cellObject
	 */
	public void analysisCellData(JSONObject cellObject, String setParam, Map<Integer, Integer> colAddCntMap,
		...
		if (cellDynamicData != null) {
			...
			for (int j = 0; j < cellDynamicData.size(); j++) {
				...
				String replace = v.replace(
						"#{".concat(dataSet.getSetCode()).concat(".").concat(dataSet.getFieldLabel()).concat("}"),
						fieldLabel);
				// 转字符串，解决深拷贝问题
	...
	public void handleStaticCellObject(String cellStr, JSONObject dbObject, int cnt, int r, int c,
			Map<String, JSONObject> cellDataMap, String setParam, JSONObject merge, Map<Integer, Integer> colAddCntMap,
			CellType cellType) {
		// 转字符串，解决深拷贝问题
		JSONObject addCellData = JSONObject.parseObject(cellStr);
		int rows = 0;
	 ...
	 * @return
	 */
	public int getRightDynamicCellRows(JSONObject addCellData, Map<String, JSONObject> cellDataMap, String setParam,
			int sumRows, CellType cellType) {
		// 1、获取此单元格右侧关联的所有单元格
	 ...
	 * @return
	 */
	public List<JSONObject> getRightDynamicCell(JSONObject addCellData, Map<String, JSONObject> cellDataMap,
			CellType cellType) {
