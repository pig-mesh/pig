...
import com.pig4cloud.pigx.common.core.jackson.PigxLongModule;
...
public class JacksonConfiguration implements WebMvcConfigurer {
     ...
     * <p>
     * 必须使用 {@link org.springframework.boot.jackson2.autoconfigure.Jackson2ObjectMapperBuilderCustomizer}
     * 而不是直接定义 {@link ObjectMapper}：
     * 直接 new ObjectMapper 会绕过 builder pipeline，导致其它模块通过 customizer 注册的能力
     * （例如 pigx-common-xss 的 String 反序列化 XSS 清洗）静默失效。
     */
    @Bean
    public org.springframework.boot.jackson2.autoconfigure.Jackson2ObjectMapperBuilderCustomizer pigxJackson2Customizer() {
        // 序列化能力按职责拆分到独立 module：
        //   PigxJavaTimeModule —— Java 8 时间类型格式化
        //   PigxLongModule     —— Long/long -> String，防 JS 精度丢失
        return builder -> builder
                .locale(Locale.CHINA)
                .timeZone(TimeZone.getTimeZone(ASIA_SHANGHAI))
                .simpleDateFormat(DatePattern.NORM_DATETIME_PATTERN)
                .modulesToInstall(new PigxJavaTimeModule(), new PigxLongModule())
                .featuresToDisable(
    ...
    /**
     * 用 Jackson 2 的 {@link org.springframework.http.converter.json.MappingJackson2HttpMessageConverter}
     * 替换 Spring Boot 4 默认注册的
     * Jackson 3 {@link JacksonJsonHttpMessageConverter}，确保 Spring MVC 走 Jackson 2 链路。
     * <p>
     * 注入的 {@link ObjectMapper} 已被所有
     * {@link org.springframework.boot.jackson2.autoconfigure.Jackson2ObjectMapperBuilderCustomizer}
     * （包括 pigx-common-xss 的 {@code xssJacksonCustomizer}）处理过，因此本 converter 同时具备
     * XSS 反序列化清洗、时间格式、Long 字符串化等能力。
     * <p>
     * 替换策略：用 {@link ListIterator#set} 原地替换 Jackson 3 converter；找不到时把 Jackson 2
     * converter 插到队首作为 fallback。不做 clear，以免破坏 byte[]、String、multipart
     * 等其它默认 converter。
