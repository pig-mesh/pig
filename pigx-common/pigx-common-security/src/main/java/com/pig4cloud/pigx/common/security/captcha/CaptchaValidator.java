package com.pig4cloud.pigx.common.security.captcha;
 ...
 *
 * <p>
 * 调用方传入 {@code captchaType} 与 {@code captchaVerification}，
 * math 类型的 {@code captchaVerification} 约定为 {@code randomStr---code}，
 * 与前端组件 {@code SmsCodeVerifyButton} / {@code SmsCodeButton} 中的拼装格式一致。
 * </p>
 *
 * @author lengleng
 * @date 2026-05-20
...
public class CaptchaValidator {
    ...
    public CaptchaResult validate(String captchaType, String captchaVerification) {
        ...
        if (StrUtil.equalsAnyIgnoreCase(captchaType, CommonConstants.IMAGE_CODE_BLOCK_PUZZLE,
            ...
            return captchaService.verification(vo).isSuccess() ? CaptchaResult.ok()
                    : CaptchaResult.failed("验证码错误或已过期");
        }
